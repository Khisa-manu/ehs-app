...
export function createSqliteApiMiddleware() {
  ...
  return async (req: any, res: any, next: any) => {
        ...
        try {
          ...
          const incMatch = pathname.match(/^\/api\/v1\/ehs\/incidents\/([^/]+)$/);
          if (incMatch && (method === 'PATCH' || method === 'PUT')) {
            const incId = incMatch[1];
            const body = await readBody();
            const status = body.status;
            const resNotes = body.resolutionNotes || body.resolution_notes || '';
            if (status) {
              db.prepare('UPDATE ehs_incidents SET status = ?, resolution_notes = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(status, resNotes, incId);
              return json({ success: true, message: 'Incident status updated' });
            }
          }

          if (pathname === '/api/v1/ehs/incidents/update-status' && method === 'POST') {
            const body = await readBody();
            const incId = body.id || body.incidentId;
            const status = body.status;
            const resNotes = body.resolutionNotes || body.resolution_notes || '';
            if (incId && status) {
              db.prepare('UPDATE ehs_incidents SET status = ?, resolution_notes = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(status, resNotes, incId);
            ...
            return json({ success: false, error: 'Incident ID and status required' }, 400);
          }
